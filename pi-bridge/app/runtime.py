...
class GatewaySettings:
    ...
    def from_environment(
        ...
        if hardware_mode not in {SIMULATOR_MODE, ROBOT_ONLY_MODE, SERIAL_MODE}:
            raise ValueError(
                "SNAP_HARDWARE_MODE must be 'simulator', 'robot-only', "
                "or 'serial', "
...
class GatewayRuntime:
    """Own the controller and the configured Arduino connections."""
    ...
    def from_settings(cls, settings: GatewaySettings) -> GatewayRuntime:
        if settings.hardware_mode in {ROBOT_ONLY_MODE, SERIAL_MODE}:
            occupancy_adapter = (
                SerialOccupancyAdapter(
    ...
    async def start(self) -> None:
        if self._started:
            return
        if self.settings.hardware_mode in {ROBOT_ONLY_MODE, SERIAL_MODE}:
            try:
                ...
                logger.exception(
                    "SNAP robot startup failed (mode=%s, robot %s @ %s).",
            ...
            if self.occupancy_adapter is not None:
                ...
                logger.info(
                    "SNAP robot-only connected: robot %s @ %s; "
                    "slot occupancy is managed from completed web jobs.",
                    self.settings.robot_port,
                    self.settings.robot_baud,
                )
        self._started = True
    ...
    def health(self) -> dict[str, object]:
        if self.settings.hardware_mode == SIMULATOR_MODE:
        ...
        robot_health = {
            ...
            "supportedOperations": ["PARKING", "RETRIEVAL"],
        }

        if self.settings.hardware_mode == ROBOT_ONLY_MODE:
            return {
                "status": "ok" if robot_connected and robot_ready else "degraded",
                "mode": "pi-hardware-robot-only",
                "hardware": {
                    "mega": {
                        "enabled": False,
                        "occupancySource": "completed-web-jobs",
                    },
                    "robot": robot_health,
                },
            }
