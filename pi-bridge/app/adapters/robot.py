...
PARKING_PATH_COMMANDS: dict[str, str] = {
    "1": "SLLLCLLCEEEOBBBCLLCKK",
    "2": "SLLLCLLWEEEOBBBWLLCK",
    "3": "SLLLCLLLLCEEEOBBBCLLLLCK",
    "4": "SLLLCLLLLWEEEOBBBWLLLLCK",
    "5": "SLLLCLLLLLLCEEEOBBBCLLLLLLCK",
    "6": "SLLLCLLLLLLWEEEOBBBWLLLLLLCK",
}

RETRIEVAL_PATH_COMMANDS: dict[str, str] = {
    "1": "LLLCLLCPSBBBCLLCEEEOK",
    "2": "LLLCLLWPSBBBWLLCEEEOK",
    "3": "LLLCLLLLCPSBBBCLLLLCEEEOK",
    "4": "LLLCLLLLWPSBBBWLLLLCEEEOK",
    "5": "LLLCLLLLLLCPSBBBCLLLLLLCEEEOK",
    "6": "LLLCLLLLLLWPSBBBWLLLLLLCEEEOK",
}

# Backwards-compatible aliases. Parking remains the default operation.
PATH_COMMANDS = PARKING_PATH_COMMANDS
...
class UnoStatusCodec:
    action_statuses: dict[str, UnoStatus] = {
        ...
        "O": UnoStatus.GRIPPING,
        "E": UnoStatus.TRACING,
        "B": UnoStatus.REVERSING,
        "K": UnoStatus.REVERSING,
    ...
    def decode(cls, frame: str | bytes) -> UnoStatus:
        ...
        if text.startswith("STATUS:RUNNING"):
            match = re.search(r"(?:^|,)COMMAND=([LWCPSBEOKH])(?:,|$)", text)
            return (
        ...
        if text.startswith("DBG:SENSOR:"):
            match = re.search(r"CMD=([LWCPSBEOKH])", text)
            return (
                cls._action_status(match.group(1), text)
                if match
                else UnoStatus.TRACING
...
def path_command_for(slot_id: str, operation: object = "PARKING") -> str:
    operation_name = _operation_name(operation)
    if operation_name == "PARKING":
        commands = PARKING_PATH_COMMANDS
    elif operation_name == "RETRIEVAL":
        commands = RETRIEVAL_PATH_COMMANDS
    else:
        name = operation_name or repr(operation)
        raise UnsupportedRobotOperationError(
            f"Robot route table does not support operation {name}.",
        )
    try:
        return commands[slot_id]
    except KeyError as error:
...
class TransportRobotAdapter:
    ...
    async def execute_path(
        ...
        command = path_command_for(slot_id, operation)
...
class SerialRobotTransport:
    ...
    async def write_command(self, command: str) -> None:
        logger.info(
            "[TX][RPI->UNO] device=%s baud=%s frame=%s",
            self.device,
            self.baudrate,
            command,
        )
    ...
    async def read_status(self) -> bytes:
        ...
        logger.info(
            "[RX][UNO->RPI] device=%s baud=%s frame=%s",
            self.device,
            self.baudrate,
...
class SerialRobotAdapter(TransportRobotAdapter):
    """Real robot adapter for automatic parking and retrieval routes."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            transport,
            codec,
            supported_operations=("PARKING", "RETRIEVAL"),
        )
    ...
    async def execute_path(
        ...
        command = path_command_for(slot_id, operation)
        route_frame = f"ROUTE:{command}"
        route_started = True
        done_seen = False
        try:
            logger.info(
                "[RPI][ROUTE_SELECTED] operation=%s slot=%s route=%s",
                _operation_name(operation),
                slot_id,
                command,
            )
            await self.transport.write_command(route_frame)
            accepted = False
            action_index = 0
